		...
		<div class="payway-box" id="pay_type" style="position: relative;">
			<div class="payway-01">
				...
				<em style='float:right;margin-right:40px;font-style:normal;' class="red">
					您需要支付&nbsp;<b style="font-size:16px;" id="need_pay"><?php echo $total_fee?></b>&nbsp;元
				</em>
			</div>
			<div class="pay-bank">
				<!-- 微信支付 -->
...
function checkStatus(){
	$.getJSON('<?php echo U("User/Pay/get_recharge_status") ?>', {trade_sn:"<?php echo $trade_sn?>"}, function(ret) {
                if(ret.status == 1) {
                    redirect('<?php echo U("User/Pay/pay_success") ?>');
                } 
            });
}
</script>
